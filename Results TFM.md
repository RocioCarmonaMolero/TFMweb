...
The final classification is found in Table 1. The classification system established by the AGNC follows the guidelines of the Human Gene Nomenclature (http://www.genenames.org/guidelines.html) that recommends its use for every specie. All this information can be found in the Aldehyde Dehydrogenase Superfamily Database (http://www.aldh.org). Since all the information is referenced and governed by the animal kingdom, the ALDH classification in plant species is subordinated to what was previously described. In turn, a protein will belong to the family and subfamily, even have the same gene number, of the ALDH already identified with which it has more homology; to facilitate next comparative studies. As an example we have the case of the CaALDH6B2 protein, which tests the non-sequential allocation of the gene number. Thus, the definition of the AGNC does not seem to meet the usual practices.
...
The phylogenetic tree shows 2 sister pairs between chickpea proteins, suggesting recent duplications after the two species separated. On the other hand we found 14 sister pairs CaALDH - MtALDH indicating that in these sequences there are no recent duplications. There is a single sister pair between *Medicago* proteins that suggests a deletion in the chickpea genome.
The ALDH18 sequences appear in a separated cluster from the rest of the sequences. The sequences of the three problematic proteins in the classification (18B3h, 18B3f and 18B3g, of short aa length) appear in a subgroup next to a MtALDH18 and CaALDH18B3d. The rest of CaALDH18 appear with their respective *Medicago* sister pairs. Thus, we hypothesize that these three *C. arietinum* ALDH18 could be the result of duplications subsequent to speciation.
Below is the cluster that corresponds to the ALDH12 sequences. Considering the function of families 12 and 18 in the same metabolic pathway, it makes sense that their sequence also differs from the rest of sequences of the analysis.
The 100% identity of the *Cicer-Medicago* sister pair in the ALDH7 sequences fits with the idea that this family is one of the most conserved eukaryotic proteins in evolution (Missihoun et al., 2014).
